...
| `EduAyudas` | Evidence-grade | Aid/scholarship official evidence consumer | `official_source_evidence` exists; `source_candidates` conversion exists; local/dev fallback exists; `aid_program` creation or publication stays outside this upstream contract. | Evidence staging and review pattern. | Clean product state; production-safe import path; small preview/import batches. | Reviewed evidence exports, official identifiers, citations, integrity, artifact availability, manual decisions. | Create `aid_programs`, publish aids, decide eligibility, or bypass review/admin flow. | Resume only after current product fixes are clean; then preview/import small batches. |
