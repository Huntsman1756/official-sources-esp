...
from official_sources.sources.dogv.client import DOGVClient, build_dogv_date_url, validate_dogv_date
from official_sources.sources.dogv.parser import NO_PUBLICATION_STATUS, parse_dogv_date_response
from official_sources.storage.repository import OfficialSourcesRepository


def ingest_dogv_date(
    ...
    *,
    ...
    run = repository.create_ingestion_run(source_code="DOGV", target_date=target_date)
    last_http_status = 200 if date_payload is not None else None
    try:
        if date_payload is None:
            if fetcher is not None:
                date_payload = _call_dogv_fetcher(fetcher, target_date)
        ...
        issue = parse_dogv_date_response(date_payload, target_date=target_date)
        if issue.status == NO_PUBLICATION_STATUS:
            run_record = repository.finish_ingestion_run(
                ...
                documents_new=0,
                documents_updated=0,
                error_message=f"DOGV returned no issue for date {target_date}",
                last_http_status=last_http_status or 200,
            )
            return _with_dogv_metadata(
                run_record,
        ...
        documents_new = 0
        documents_updated = 0
        raw_url = build_dogv_date_url(target_date)
...
def _call_dogv_fetcher(fetcher: Callable, target_date: str) -> bytes:
    return fetcher(target_date)


def _with_dogv_metadata(
    run_record: dict,
    *,
