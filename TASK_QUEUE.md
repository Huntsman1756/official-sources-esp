...
| `TASK-PROVINCIAL-MONITORS-WAVE-003` | implemented locally | Adds `BOP_SEVILLA` as a single metadata-only HTML monitor under the partial-health contract. Live preview on 2026-05-27 returned `records=1` with `not_candidate`, `not_evidence`, and `unclassified`; parser validation extracted 33 live records. `BOP_ALICANTE` remains degraded/manual-review and excluded from all-sources-green claims. No PDFs/artifacts, candidates, evidence-grade records, downstream writes, broad scraping/backfill, VPS, Hermes, systemd, or timer changes. |
