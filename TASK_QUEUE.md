...
| `TASK-PROVINCIAL-MONITORS-WAVE-003` | blocked | Consider at most 1-2 further metadata-only pilots only after `BOP_ALICANTE` live health recovers or it is explicitly excluded from the combined healthy set by a separate scoped decision. `BOP_SEVILLA` remains the next prior audit candidate. Keep source-specific parsers, fixtures/fetchers, preview validation, and no publication/candidate/evidence/PDF/downstream writes. |
