...
| `TASK-PROVINCIAL-MONITORS-WAVE-002` | proposed | Consider the next 1-2 metadata-only pilots from the read-only audit evidence, likely `BOP_BIZKAIA` and `BOP_VALENCIA`, only after wave 001 is reviewed/merged. Keep source-specific parsers, fixtures/fetchers, preview validation, and no publication/candidate/evidence/PDF/downstream writes. |
